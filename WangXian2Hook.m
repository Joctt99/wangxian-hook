            ...
            for (uint32_t s = 0; s < dataSegs[d]->nsects; s++) {
                ...
                for (uint32_t j = 0; j < count; j++) {
                    ...
                    if (strcmp(name, symbolName) == 0) {
                        void *page = (void *)((uintptr_t)&pointers[j] & ~(16384 - 1));
                        if (mprotect(page, 16384, PROT_READ | PROT_WRITE) == 0) {
...
static void installSocketHooks(void) {
    rebindSymbol("_recv", (void *)hook_recv, (void **)&orig_recv);
    rebindSymbol("_recvfrom", (void *)hook_recvfrom, (void **)&orig_recvfrom);
    rebindSymbol("_recvmsg", (void *)hook_recvmsg, (void **)&orig_recvmsg);
    
    if (!orig_recv) orig_recv = (RecvFunc)dlsym(RTLD_NEXT, "recv");
    if (!orig_recvfrom) orig_recvfrom = (RecvfromFunc)dlsym(RTLD_NEXT, "recvfrom");
    if (!orig_recvmsg) orig_recvmsg = (RecvmsgFunc)dlsym(RTLD_NEXT, "recvmsg");
    
    DLOG(@"[SOCK] Hooks installed: recv=%p recvfrom=%p recvmsg=%p", orig_recv, orig_recvfrom, orig_recvmsg);
}
