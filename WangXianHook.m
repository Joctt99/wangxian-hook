/**
 * WangXianHook v34.27 - Anti-Cheat Bypass + DYLD Hiding + Protocol Login Patch
...
static void log_init(void) {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:p]) {
        g_logPath = p;
        _log(@"=== WXHook v34.27 Full Protocol Patch ===");
...
static ssize_t hook_send(int fd, const void *buf, size_t len, int flags) {
    ...
    if (port == 5678 && len >= 12) {
        ...
        if (cmd == 0x0002A018) {
            // Original format: "UUID=" + "MACADDRESS=" + 36-byte UUID value
            // Example: UUID=MACADDRESS=180C4F27-4414-4623-ACEB-0C12B30E48FD
            for (size_t i = 0; i + 52 <= len; i++) {  // 5+11+36=52 bytes minimum
                if (memcmp(p + i, "UUID=MACADDRESS=", 16) == 0) {
                    // Original data to replace: "UUID=" + "MACADDRESS=" + 36-byte UUID = 52 bytes
                    size_t oldDataLen = 52;  // "UUID="(5) + "MACADDRESS="(11) + UUID value(36)
                    
                    // New data: UUID value + null + MACADDRESS + MAC value
                    const char replacement[] = "UUID=12345678-1234-1234-1234-123456789012\x00MACADDRESS=00:11:22:33:44:55";
                    size_t replaceLen = sizeof(replacement) - 1;  // 67 bytes
                    
                    size_t diff = replaceLen - oldDataLen;  // 67 - 52 = 15
                    
                    void *newBuf = malloc(len + diff);
                    if (newBuf) {
                        // Copy header (before UUID field)
                        memcpy(newBuf, buf, i);
                        
                        // Copy replacement data
                        memcpy(((char *)newBuf) + i, replacement, replaceLen);
                        
                        // Copy tail (after old UUID value)
                        memcpy(((char *)newBuf) + i + replaceLen, 
                               ((char *)buf) + i + oldDataLen,
                               len - i - oldDataLen);
                        
                        // Update packet length (4 bytes big endian)
                        ...
                        sendBuf = newBuf;
                        sendLen = len + diff;
                        DLOG(@"[SEND-PATCH] Filled UUID/MAC for cmd=0x%08X (old=%zu new=%zu diff=%zu)", cmd, len, sendLen, diff);
                    }
                    break;
                }
            }
