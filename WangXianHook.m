/**
 * WangXianHook v35.56 - Fix mock server list pktLen mismatch causing "数据包太大" error
 * FIX: Updated pktLen from 88 to 109 (0x6D) to match actual data length
 * FIX: Fixed IP format - now single string "139.224.129.92" instead of split format
 * FIX: Fixed port format - now single string "12003" instead of split format
 * ROOT CAUSE: v35.55 injected 109 bytes but pktLen=88, causing length mismatch and disconnect
...
static ssize_t hook_recv(int fd, void *buf, size_t len, int flags) {
    ...
    if (ret >= 8) {
        ...
        // v35.56: Handle 0x80000015 response - inject mock server list with correct pktLen
        // PROBLEM: Server echoes back 0x80000015 with pktLen=22, but we inject 109 bytes
        // SOLUTION: Replace entire packet including cmd and pktLen, update pktLen to match actual data
        if (cmd == 0x80000015 && port == 5678) {
            DLOG(@"[PROTO-R] 0x80000015 response received, injecting mock server list");
            ...
            if (len >= mockLen) {
                ...
                DLOG(@"[PROTO-R-PATCH] Injected mock server list: cmd=0x802EE118 pktLen=109 (overwrote %zd bytes)", ret);
                ...
                if (len > 8) {
                    ...
                    p[2] = (mockLen >> 8) & 0xFF;
                    p[3] = mockLen & 0xFF;
                    p[4] = 0x80; p[5] = 0x2E; p[6] = 0xE1; p[7] = 0x18;
                    size_t copyLen = len < mockLen ? len : mockLen;
                    memcpy(p, mockServerList, copyLen);
                    DLOG(@"[PROTO-R-PATCH] Partial injection: updated header, copied %zu bytes", copyLen);
...
static BOOL hook_APEX_isJailbroken(id self, SEL _cmd) {
    BOOL orig = orig_APEX_isJailbroken ? orig_APEX_isJailbroken(self, _cmd) : YES;
    static int g_logCount = 0;
    if (g_logCount < 3) {
        DLOG(@"[APEX] isJailbroken: %d -> NO", orig);
        g_logCount++;
    }
    return NO;
}

static NSString *(*orig_APEX_clientKey)(id, SEL);
static NSString *hook_APEX_clientKey(id self, SEL _cmd) {
    NSString *orig = orig_APEX_clientKey ? orig_APEX_clientKey(self, _cmd) : nil;
    static int g_logCount = 0;
    if (g_logCount < 3) {
        DLOG(@"[APEX] clientKey: %@", orig);
        g_logCount++;
    }
    return orig;
}

static NSString *(*orig_APEX_tid)(id, SEL);
static NSString *hook_APEX_tid(id self, SEL _cmd) {
    NSString *orig = orig_APEX_tid ? orig_APEX_tid(self, _cmd) : nil;
    static int g_logCount = 0;
    if (g_logCount < 3) {
        DLOG(@"[APEX] tid: %@", orig);
        g_logCount++;
...
static NSString *(*orig_APEX_udid)(id, SEL);
static NSString *hook_APEX_udid(id self, SEL _cmd) {
    NSString *orig = orig_APEX_udid ? orig_APEX_udid(self, _cmd) : nil;
    static int g_logCount = 0;
    if (g_logCount < 3) {
        DLOG(@"[APEX] udid: %@", orig);
        g_logCount++;
...
static NSString *(*orig_APEX_virtualImei)(id, SEL);
static NSString *hook_APEX_virtualImei(id self, SEL _cmd) {
    NSString *orig = orig_APEX_virtualImei ? orig_APEX_virtualImei(self, _cmd) : nil;
    static int g_logCount = 0;
    if (g_logCount < 3) {
        DLOG(@"[APEX] virtualImei: %@", orig);
        g_logCount++;
    }
    return orig;
}

static NSString *(*orig_APEX_virtualImsi)(id, SEL);
static NSString *hook_APEX_virtualImsi(id self, SEL _cmd) {
    NSString *orig = orig_APEX_virtualImsi ? orig_APEX_virtualImsi(self, _cmd) : nil;
    static int g_logCount = 0;
    if (g_logCount < 3) {
        DLOG(@"[APEX] virtualImsi: %@", orig);
        g_logCount++;
    }
    return orig;
}
