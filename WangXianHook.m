/**
 * WangXianHook v34.40 - Anti-Cheat Bypass + DYLD Hiding + Protocol Login Patch
...
static void log_init(void) {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:p]) {
        g_logPath = p;
        _log(@"=== WXHook v34.40 Full Protocol Patch ===");
...
static ssize_t hook_recv(int fd, void *buf, size_t len, int flags) {
    ...
    if (port == 5678 && ret >= 13) {
        ...
        if (cmd == 0x00FFFF01 || cmd == 0x00FFFF02) {
            ...
            // Fake login response: 0x8002A017 with status=0
            const char fakeLoginResp[] = 
                ...
                "\x00\x00\x00\x00"  // more status fields
                "\x00\x00\x00\x00"
                "\x00\x00\x00\x00"
                "\x00\x00\x00\x00"
                "\x00\x00\x00\x00"
        ...
        if (cmd == 0x76666669 || cmd == 0x7666669A) {  // 'vffi' or 'vfii'
            ...
            // Fake login success: 0x8002A017 with status=0
            const char fakeLoginResp[] = 
                "\x00\x00\x00\x25"  // length = 37
                "\x80\x02\xA0\x17"  // cmd = 0x8002A017 (login response)
                "\x00\x00\x00\x00"  // status = 0 (success)
