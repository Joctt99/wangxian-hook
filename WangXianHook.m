/**
 * WangXianHook v34.35 - Anti-Cheat Bypass + DYLD Hiding + Protocol Login Patch
...
static void log_init(void) {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:p]) {
        g_logPath = p;
        _log(@"=== WXHook v34.35 Full Protocol Patch ===");
...
static ssize_t hook_recv(int fd, void *buf, size_t len, int flags) {
    ...
    if (port == 5678 && ret >= 13) {
        ...
        if (cmd == 0x802EE113) {
            ...
            // Also patch the server status in JSON string (status=6 -> status=1)
            // Find "status=6" and change to "status=1"
