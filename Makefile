...
$(TARGET): $(SOURCE) $(PROTO) $(FISHHOOK)
	...
	@grep -c "FIX53M" $(SOURCE) || echo "WARNING: FIX53M not found in source!"
	@grep -c "FIX53M-REENC" $(SOURCE) || echo "WARNING: FIX53M-REENC not found!"
	@grep -c "FIX53M-SKIP" $(SOURCE) || echo "WARNING: FIX53M-SKIP not found!"
	@grep -c "FIX53M-DETECT" $(SOURCE) || echo "WARNING: FIX53M-DETECT not found!"
	...
	@strings $(TARGET) | grep -c "FIX53M" || echo "WARNING: FIX53M not in binary!"
	@strings $(TARGET) | grep -c "FIX53M-REENC" || echo "WARNING: FIX53M-REENC not in binary!"
	@strings $(TARGET) | grep -c "FIX53M-SKIP" || echo "WARNING: FIX53M-SKIP not in binary!"
